...
CREATE INDEX `idx_memory_review_pending` ON `memory_review` (`tenant_id`,`reviewer`,`reviewed_at` DESC) WHERE status = 'unreviewed';--> statement-breakpoint
CREATE INDEX `recall_traces_target` ON `memory_recall_traces` (`tenant_id`,`target_id`,`at`);
